...
| `OCI_SPEECH_MODEL` | No | `whisper-medium` | Set in root `.env` when selecting a non-default speech model. | Set as a runtime environment variable when selecting a non-default speech model. | Accepted values: `whisper-medium`, `whisper-large-v3-turbo`. |
